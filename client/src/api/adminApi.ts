...
export const getAllDeliveryBoys = async (params: {
    ...
    try {
        ...
        console.log('API Call URL:', `/admin/delivery-boys?${queryParams.toString()}`);
        ...
        const response = await privateApi('get', `/admin/delivery-boys?${queryParams.toString()}`);
...
export const unBlockCustomer = async (customerId: string) => {
  const response = await privateApi('patch',`/admin/user/${customerId}/unblock`)
  return response
}
